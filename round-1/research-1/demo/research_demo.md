...
[1] [MinHash - Wikipedia](https://en.wikipedia.org/wiki/MinHash) — Documents the variance formula Var[Ĵ] = J(1-J)/k for standard MinHash estimator and the expected error O(1/√k), meaning k = O(1/ε²) hashes needed for error ε.

[2] [MinHash - Wikipedia (Broder 1997, 2000)](https://en.wikipedia.org/wiki/MinHash) — Original MinHash papers by Broder introducing the technique for estimating Jaccard similarity using minimum hash values under random permutations.

[3] [Is Min-Wise Hashing Optimal for Summarizing Set Intersection? (Pagh, Stöckel, Woodruff, 2014)](https://www.cs.cmu.edu/~afs/cs/user/dwoodruf/www/psw14.pdf) — Provides information-theoretic lower bound Ω((1-J)/(εJ)²) bits per set for estimating Jaccard similarity with relative error εJ. Shows b-bit minwise hashing is space optimal for m=2 predicates.

[4] [Theory and applications of b-bit minwise hashing (Li & König, 2011)](https://dl.acm.org/doi/10.1145/1978542.1978566) — Introduces b-bit MinHash that reduces storage by storing only lowest b bits of each minhash. The compression is lossy and irreversible.

[5] [Efficient estimation for high similarities using odd sketches (Mitzenmacher, Pagh, Pham, 2014)](https://www.rasmuspagh.net/papers/oddsketch.pdf) — Gives the variance formula for b-bit MinHash: Var[Ĵ_b] = (1-J)/k · (J + 1/(2^b - 1)). Also introduces Odd Sketch for high Jaccard similarities.

[6] [SetSketch: Filling the Gap between MinHash and HyperLogLog (Ertl, 2021)](https://vldb.org/pvldb/vol14/p2244-ertl.pdf) — Documents that SetSketch has 4 parameters (m, b, a, q) that must be set before sketch construction (Section 2.3). While filling the gap between MinHash and HLL, it does not support progressive refinement.

[7] [Odd Sketch for high Jaccard similarities (Mitzenmacher et al., 2014)](https://www.rasmuspagh.net/papers/oddsketch.pdf) — Describes Odd Sketch construction using parity (XOR) of hash values mapped to bits. The sketch size n must be fixed upfront. Estimator accuracy depends on fraction of 1s being bounded away from 1/2.

[8] [ProbMinHash - A Class of Locality-Sensitive Hash Algorithms for the Probability Jaccard Similarity (Ertl, 2020)](https://arxiv.org/abs/1911.00675) — Extends MinHash to weighted Jaccard similarity. Generates a finite signature of k hash values. The algorithm produces a fixed-size signature, not rateless.

[9] [LT Codes (Luby, 2002)](https://www.inference.org.uk/mackay/dfountain/LT.pdf) — Describes LT codes encoding process: choose degree d from distribution ρ(d), select d distinct input symbols, XOR to produce encoding symbol. Decoding uses belief propagation. The code is rateless—encoding symbols can be generated indefinitely.

[10] [Raptor Codes (Shokrollahi, 2006)](https://www.qualcomm.com/content/dam/qcomm-martech/dm-assets/documents/Raptor_Codes_IEEE_technical_analysis.pdf) — Improves upon LT codes with linear time encoding and decoding by applying a fixed-rate outer code before LT coding.

[11] [Elements of Information Theory - Chapter 13: Rate Distortion Theory (Cover & Thomas, 1991)](https://static.ias.edu/pitp/archive/2012files/Cover_and_Thomas_Chptr13.pdf) — Provides the rate-distortion function for Bernoulli source with Hamming distortion: R(D) = H_b(p) - H_b(D) for 0 ≤ D ≤ min(p, 1-p).

[12] [Rate-distortion theory - Wikipedia](https://en.wikipedia.org/wiki/Rate%E2%80%93distortion_theory) — Documents the R(D) formula for Bernoulli source with Hamming distortion. Also provides the general rate-distortion function definition and optimization problem.

[13] [On optimal stopping problems in sequential hypothesis testing (Lai, 1997)](https://www3.stat.sinica.edu.tw/statistica/password.asp?vol=7&num=1&art=2) — Discusses optimal stopping rules for sequential hypothesis testing, including SPRT which minimizes expected sample size while bounding error probabilities.

[14] [Intelligent Probing for Locality Sensitive Hashing (VLDB 2017)](http://vldb.org/pvldb/vol10/p2021-lv.pdf) — Describes multi-probe LSH that intelligently probes multiple buckets likely to contain query results. Adaptive but not progressive in the Rateless MinHash sense.

[15] [Sketch Techniques for Approximate Query Processing](https://dimacs.rutgers.edu/~graham/pubs/papers/sk.pdf) — Reviews sketching techniques for approximate query processing. Some techniques provide initial estimates that improve with more data, but not applied to MinHash or progressive Jaccard estimation.
